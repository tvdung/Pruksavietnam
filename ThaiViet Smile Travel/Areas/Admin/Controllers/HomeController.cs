...
namespace ThaiVietSmileTravel.Areas.Admin.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult CreateTour()
        {
            ...
            list.Add(new SelectListItem { Text = "Chọn danh mục", Value = "0" });//set default text for DropdownList
            var cat = (from c in db.tbl_Categories select c).ToArray();//get data from database using linq
            for (int i = 0; i < cat.Length; i++)
            {
                list.Add(new SelectListItem { Text = cat[i].TenVN, Value = cat[i].Id.ToString() });
            }
        ...
        public ActionResult EditCategory(int? id)
        {
            ...
            tbl_Categories tbl_Category = db.tbl_Categories.Find(id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditCategory([Bind(Include = "Id,TenTL,TenVN,TenEN,HinhAnh,IsActive")] tbl_Categories tbl_Category)
        {
            if (ModelState.IsValid)
            {
                db.Entry(tbl_Category).State = EntityState.Modified;
                db.SaveChanges();
                var model = db.tbl_Categories.ToList();
                return RedirectToAction("Category", model);
            }
            return View(tbl_Category);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateCategory([Bind(Include = "Id,TenTL,TenVN,TenEN,HinhAnh,IsActive")] tbl_Categories tblCategories)
        {
            if (ModelState.IsValid)
            {
                db.tbl_Categories.Add(tblCategories);
                db.SaveChanges();
